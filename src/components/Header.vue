  ...
  <div>
    ...
    <header class="header container">
      ...
      <div class="h_con">
        <ul class="clearfloat">
          ...
          <li v-show="!$store.state.uid">
            <router-link to="/login" class="h_login">登录</router-link>
          </li>
          <li v-show="!$store.state.uid"><em>|</em></li>
          <li v-show="!$store.state.uid"><router-link to="/register" class="h_register">注册</router-link></li>
          <li v-show="$store.state.uid" class="top_user"><a href="">{{$store.state.uname}}</a></li>
          <li v-show="$store.state.uid" class="top_quit"><a @click="$store.commit('clearUser')">退出</a></li>
        </ul>
    ...
    <nav class="navbar">
      <div class="container">
        <div class="nav">
          <ul>
            ...
            <li><router-link to="/news"
              :class="{'router-link-exact-active': $route.name == 'NewsDetails'}">公司动态</router-link></li>
            <li class="nav_pd">
              <router-link to="/product/1"
              :class="{
                'router-link-exact-active': $route.name == 'Product'
                }">产品中心<span class="icon_pd"></span></router-link>
        ...
        <div class="s_cart" v-show="$store.state.uid">
          <router-link to="/cart">
