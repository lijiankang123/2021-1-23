  ...
  <div>
    ...
    <div class="main container" v-if="data">
        <div class="news_details">
            <h2>{{data.title}}</h2>
            <span>发布时间：{{moment(data.pubTime * 1).format("YYYY-MM-DD")}}</span>
...
export default {
  ...
  methods:{
    getData(){
      ...
      const url = "news_detail.php?nid=" + nid;
      // console.log(url);
      this.axios.get(url)
        .then(res => {
          // res.data.content = res.data.content
          // .replace(/<img src="/g,'<img src="http://101.96.128.94:9999/mfresh/"');
